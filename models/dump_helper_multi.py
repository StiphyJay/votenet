...
def dump_results(end_points, dump_dir, config, inference_switch=False):
    ...
    for i in range(batch_size):
        ...
        pc_util.write_ply(pc, os.path.join(dump_dir, '{}_pc.ply'.format(end_points['scan_name'][i])))
        pc_util.write_ply(seed_xyz[i,:,:], os.path.join(dump_dir, '{}_seed_pc.ply'.format(end_points['scan_name'][i])))
        if 'vote_xyz' in end_points:
            pc_util.write_ply(end_points['vote_xyz'][i,:,:].view(-1, 3), os.path.join(dump_dir, '{}_vgen_pc.ply'.format(end_points['scan_name'][i])))
            pc_util.write_ply(aggregated_vote_xyz[i,:,:], os.path.join(dump_dir, '{}_aggregated_vote_pc.ply'.format(end_points['scan_name'][i])))
            pc_util.write_ply(aggregated_vote_xyz[i,:,:], os.path.join(dump_dir, '{}_aggregated_vote_pc.ply'.format(end_points['scan_name'][i])))
        pc_util.write_ply(pred_center[i,:,0:3], os.path.join(dump_dir, '{}_proposal_pc.ply'.format(end_points['scan_name'][i])))
        if np.sum(objectness_prob>DUMP_CONF_THRESH)>0:
            pc_util.write_ply(pred_center[i,objectness_prob>DUMP_CONF_THRESH,0:3], os.path.join(dump_dir, '{}_confident_proposal_pc.ply'.format(end_points['scan_name'][i])))
    ...
    for i in range(batch_size):
        if np.sum(objectness_label[i,:])>0:
            pc_util.write_ply(pred_center[i,objectness_label[i,:]>0,0:3], os.path.join(dump_dir, '{}_gt_positive_proposal_pc.ply'.format(end_points['scan_name'][i])))
        if np.sum(objectness_mask[i,:])>0:
            pc_util.write_ply(pred_center[i,objectness_mask[i,:]>0,0:3], os.path.join(dump_dir, '{}_gt_mask_proposal_pc.ply'.format(end_points['scan_name'][i])))
        pc_util.write_ply(gt_center[i,:,0:3], os.path.join(dump_dir, '{}_gt_centroid_pc.ply'.format(end_points['scan_name'][i])))
        pc_util.write_ply_color(pred_center[i,:,0:3], objectness_label[i,:], os.path.join(dump_dir, '{}_proposal_pc_objectness_label.obj'.format(end_points['scan_name'][i])))
        ...
        if len(obbs)>0:
            ...
            pc_util.write_oriented_bbox(obbs, os.path.join(dump_dir, '{}_gt_bbox.ply'.format(end_points['scan_name'][i])))

    # OPTIONALL, also dump prediction and gt details
    if 'batch_pred_map_cls' in end_points:
        for ii in range(batch_size):
            fout = open(os.path.join(dump_dir, '{}_pred_map_cls.txt'.format(end_points['scan_name'][ii])), 'w')
    ...
    if 'batch_gt_map_cls' in end_points:
        for ii in range(batch_size):
            fout = open(os.path.join(dump_dir, '{}_gt_map_cls.txt'.format(end_points['scan_name'][ii])), 'w')
