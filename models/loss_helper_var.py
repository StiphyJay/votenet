...
def compute_vote_loss(end_points):
    ...
    vote_var = end_points['vote_var'] # B,num_seed*vote_factor,3
    ...
    dist1, _, dist2, _ = nn_distance(vote_xyz_reshape, seed_gt_votes_reshape, l1smooth=True, ifsum=False)
    votes_dist = dist2[:, 0, :].view(batch_size, num_seed, -1)
    vote_loss = torch.sum((torch.exp(-vote_var)*votes_dist + 0.5 * vote_var)*seed_gt_votes_mask.unsqueeze(-1).float())/(torch.sum(seed_gt_votes_mask.float())+1e-6)
    return vote_loss
