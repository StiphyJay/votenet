...
class ProposalModuleCls(nn.Module):
    ...
    def forward(self, xyz, features, end_points):
        ...
        # pruning
        _, vote_best_n_inds = torch.topk(end_points['vote_pruning_key'], self.vote_config.best_n_votes, dim=1, sorted=self.sorted_clustering)
        end_points['vote_best_n_inds'] = vote_best_n_inds
        vote_best_n_inds_expand = vote_best_n_inds.unsqueeze(-1).repeat(1,1,3)
        xyz = torch.gather(xyz, 1, vote_best_n_inds_expand).contiguous() # (B, best_n, 3)
        end_points['vote_best_n_xyz'] = xyz
        vote_best_n_inds_expand = vote_best_n_inds.unsqueeze(1).repeat(1,features.size(1),1)
        features = torch.gather(features, 2, vote_best_n_inds_expand).contiguous() # (B, C, best_n)
        ...
        if self.sampling == 'vote_fps':
            ...
            # FPS on seed and choose the votes corresponding to the seeds
            # This gets us a slightly better coverage of *object* votes than vote_fps (which tends to get more cluster votes)
            seed_best_n_xyz = torch.gather(end_points['seed_xyz'], 1, vote_best_n_inds_expand)
            sample_inds = pointnet2_utils.furthest_point_sample(seed_best_n_xyz, self.num_proposal) # (batch_size, num_proposal)

            # map index to votes (batch_size, num_proposal)
            xyz, features, _ = self.vote_aggregation(xyz, features, sample_inds)
        elif self.sampling == 'random':
            # Random sampling from the votes
            sample_inds = torch.randint(0, num_seed*self.num_vote, (batch_size, self.num_proposal), dtype=torch.int).cuda()
            xyz, features, _ = self.vote_aggregation(xyz, features, sample_inds)
        else:
            log_string('Unknown sampling strategy: %s. Exiting!'%(self.sampling))
            exit()
...
class ProposalModuleMulti(nn.Module):
    ...
    def forward(self, xyz, features, end_points):
        ...
        if self.sampling == 'vote_fps':
            ...
            xyz, features, fps_inds = self.vote_aggregation(xyz, features)
            sample_inds = fps_inds
        elif self.sampling == 'seed_fps': # TODO: seems this don't support multi votes
            # FPS on seed and choose the votes corresponding to the seeds
            # This gets us a slightly better coverage of *object* votes than vote_fps (which tends to get more cluster votes)
            sample_inds = pointnet2_utils.furthest_point_sample(end_points['seed_xyz'], self.num_seed_fps) # (batch_size, num_proposal / num_vote)
...
if __name__=='__main__':
    ...
    print()
    net = ProposalModuleMulti(DC.num_class, DC.num_heading_bin,
                              DC.num_size_cluster, DC.mean_size_arr,
                              256*3, 'seed_fps').cuda()

    out = net(torch.rand(8,1024*3,3).cuda(), torch.rand(8,256,1024*3).cuda(), end_points)
    for key in out:
        print(key, out[key].shape)