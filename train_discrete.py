...
from model_util_vote import VoteConfig_Discrete_Polar, VoteConfig_Discrete_Grid
from pytorch_utils import BNMomentumScheduler
from tf_visualizer import Visualizer as TfVisualizer
from ap_helper import APCalculator, parse_predictions, parse_groundtruths

parser = argparse.ArgumentParser()
parser.add_argument('--model', default='votenet_discrete', help='Model file name [default: votenet_discrete]')
...
parser.add_argument('--polar_vote', default=False, action='store_true', help='Use grid votes')
parser.add_argument('--polar_vote_num_heading_bin', default=None, type=int)
parser.add_argument('--polar_vote_fixed_votes_r', default=None, type=str)
parser.add_argument('--polar_vote_fixed_votes_z', default=None, type=str)

parser.add_argument('--grid_vote', default=False, action='store_true', help='Use grid votes')
parser.add_argument('--grid_vote_fixed_votes_x', default=None, type=str)
parser.add_argument('--grid_vote_fixed_votes_y', default=None, type=str)
parser.add_argument('--grid_vote_fixed_votes_z', default=None, type=str)
...
assert FLAGS.grid_vote != FLAGS.polar_vote
if FLAGS.grid_vote:
    fixed_votes_x = [float(x) for x in FLAGS.grid_vote_fixed_votes_x.split(',')]
    fixed_votes_y = [float(y) for y in FLAGS.grid_vote_fixed_votes_y.split(',')]
    fixed_votes_z = [float(z) for z in FLAGS.grid_vote_fixed_votes_z.split(',')]
    VC = VoteConfig_Discrete_Grid(fixed_votes_x=fixed_votes_x,
                                  fixed_votes_y=fixed_votes_y,
                                  fixed_votes_z=fixed_votes_z,
                                  top_n_votes=FLAGS.top_n_votes)
else:
    fixed_votes_r = [float(r) for r in FLAGS.polar_vote_fixed_votes_r.split(',')]
    fixed_votes_z = [float(z) for z in FLAGS.polar_vote_fixed_votes_z.split(',')]
    VC = VoteConfig_Discrete_Polar(num_heading_bin=FLAGS.polar_vote_num_heading_bin,
                                   fixed_votes_r=fixed_votes_r,
                                   fixed_votes_z=fixed_votes_z,
                                   top_n_votes=FLAGS.top_n_votes)
...
if FLAGS.dataset == 'sunrgbd':
    ...
    from scannet_detection_dataset_discrete import ScannetDetectionDatasetDiscrete, MAX_NUM_OBJ
    from model_util_scannet import ScannetDatasetConfig
    DATASET_CONFIG = ScannetDatasetConfig()
    TRAIN_DATASET = ScannetDetectionDatasetDiscrete('train', num_points=NUM_POINT,
        augment=True,
        use_color=FLAGS.use_color, use_height=(not FLAGS.no_height),
        vote_config=VC)
    TEST_DATASET = ScannetDetectionDatasetDiscrete('val', num_points=NUM_POINT,
        augment=False,
        use_color=FLAGS.use_color, use_height=(not FLAGS.no_height),
        vote_config=VC)
else:
...
elif FLAGS.model == 'votenet_multi':
    Detector = MODEL.VoteNetMulti
else:
    Detector = MODEL.VoteNetDiscrete
